...
// How long a TEARDOWN is given to leave the socket before it is forced shut.
const TEARDOWN_FLUSH_MS = 1000;
...
class RtspClient extends EventEmitter {
  ...
  close() {
    ...
    if (this.session && this.sock && !this.sock.destroyed) {
      // TEARDOWN has to reach the camera, or it keeps the session allocated
      // until its own timeout and the next viewer can be refused. destroy()
      // discards whatever is still buffered, so the write is flushed with
      // end() and the socket is only forced shut if that does not complete.
      try { this.sock.write(this._format("TEARDOWN", this.url, {})); } catch {}
      try { this.sock.end(); } catch {}
      const sock = this.sock;
      const timer = setTimeout(() => { try { sock.destroy(); } catch {} }, TEARDOWN_FLUSH_MS);
      if (timer.unref) timer.unref();
      this._close(null, { flushing: true });
      return;
    }
    this._close(null);
  }
  ...
  _close(err, { flushing = false } = {}) {
    ...
    // `flushing`: close() is letting a TEARDOWN drain and will force the socket
    // shut itself. Every other path drops it now.
    if (!flushing) { try { this.sock && this.sock.destroy(); } catch {} }
