// test/connectors/bambu-preview.test.js â€” the job preview for Bambu Lab
...
test("preview: cached per job â€” asking again only checks the file's size, it does not read it again", async () => {
...
test("preview: a job whose file is not on the printer is a 404, remembered for a while", async () => {
  await withPrinter({ files: {} }, async (p, ftp) => {
    ...
    await assert.rejects(bambu.getThumbnail(p, "Bracket v4"), e => e.status === 404);
    assert.equal(ftp.state.logins, logins, "the miss is cached â€” the card's retries cost nothing");
  });
});
...
test("preview: the data connection goes to the verified host, never to the address in the PASV reply", async () => {
  ...
  // reach it â€” success proves the advertised host was ignored.
