// test/connectors/bambu-camera.test.js â€” the Bambu Lab camera relay: RTP
...
// decoded by ffmpeg â€” the independent proof that what goes to the browser is
...
test("relay: two viewers share ONE camera session, which closes after the last leaves", async () => {
  await withPrinter({}, async (p, cam) => {
    ...
    // Polled rather than slept on: the relay closes after its idle delay and
    // TEARDOWN has to travel and be parsed, so a fixed wait made this fail
    // about one run in three even when the code was right.
    const deadline = Date.now() + 4000;
    while (Date.now() < deadline &&
           !(cam.state.sockets.size === 0 && cam.state.requests.some(r => r.method === "TEARDOWN"))) {
    ...
    assert.ok(cam.state.requests.some(r => r.method === "TEARDOWN"),
      "without TEARDOWN the camera holds the session until its own timeout");
  });
});
...
test("relay: a malformed SPS ends that camera session â€” the process keeps running", async () => {
