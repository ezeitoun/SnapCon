...
// `commandReply` decides how the printer answers a print.* command:
//   "success"      — result SUCCESS, the verified reply from a P2S with
//                    Developer Mode on
//   "verifyFailed" — result failed / reason "mqtt message verify failed", the
//                    verified reply with Developer Mode off
//   "silent"       — no reply at all (a printer that ignores the command)
function createFakeBambuBroker({ serial, accessCode, report, version, commandReply = "success", createServer = (h) => net.createServer(h) } = {}) {
  const state = {
    ...
    sock.on("data", (chunk) => {
      ...
      for (const pk of packets) {
        if (pk.type === mq.TYPE.CONNECT) {
          ...
          // A print command is answered on the REPORT topic, the way the real
          // printer does, echoing the sequence_id it was given.
          if (json && json.print && json.print.command && json.print.command !== "push_status" && state.commandReply !== "silent") {
            const seq = json.print.sequence_id;
            const cmd = json.print.command;
            setTimeout(() => {
              const body = state.commandReply === "verifyFailed"
                ? { command: cmd, sequence_id: seq, result: "failed", reason: "mqtt message verify failed" }
                : { command: cmd, sequence_id: seq, result: "SUCCESS", reason: "SUCCESS" };
              sock.write(mq.encodePublish(`device/${serial}/report`, JSON.stringify({ print: body })));
            }, 5);
          }
